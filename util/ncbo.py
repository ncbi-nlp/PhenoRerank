...
def annotext(text, ontos=[]):
	...
	try:
		...
		res = [dict(id=annot['annotatedClass']['@id'].split('/')[-1], url=annot['annotatedClass']['@id'], loc=(antxt['from']-1, antxt['to']), text=antxt['text'], mtype=antxt['matchType']) for annot in ret_data for antxt in annot['annotations']]
